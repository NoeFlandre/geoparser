...
- **Fine-tune a module** for a language, period, or domain the defaults handle poorly — see [training](training.md)
...
If you already have annotations in another format, you do not need the application either: `create_references()` and `create_referents()` take spans and identifiers directly, as shown in [projects](projects.md). The annotator is for producing them by hand.
...
From here the annotations are ordinary project data: they can train a module, as in [training](training.md), or serve as the reference set for evaluation.
